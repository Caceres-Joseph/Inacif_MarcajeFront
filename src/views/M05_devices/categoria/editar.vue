  ...
  <!-- Categoría padre -->
                ...
                <v-flex xs12>
                    <v-select
                        ...
                        :label="seleccioneCagegoriaPadre"
                        autocomplete  
                        @change="cbCambio"
                        :disabled="!enabled"
                        chips
...
export default {
  ...
  data() {
    return {
      ...
      nameRules: [v => !!v || "El nombre de la categoría es obligatorio"],
      valid: false,

      getCategorias: [],
      seleccioneCagegoriaPadre: "Seleccione"
    };
  },
  methods: {
    addItem() {
      let uri = this.ip + `categoria_update/${
      ...
      if (this.image == null) {
        ...
        if (!this.files2.length) {
          //solo esta subido como ruta
          this.item.imagen = this.getCategorias.imagen;
        } else {
    ...
    getItem() {
      ...
      let uri = this.ip + `categoria_getitem/${
        this.$route.params.id
      }`;
      this.axios.get(uri).then(response => {
        this.getCategorias = response.data;
        //this.$log.info( this.getCategorias);

        this.item.nombre = this.getCategorias.nombre;
        this.item.descripcion = this.getCategorias.descripcion;
        if (this.getCategorias.idCategoriaPadre != -1) {
          //no tiene padre
          this.enabled = true;
          this.a1 = this.getCategorias.idCategoriaPadre;
          this.idCategoriaPadre = this.getCategorias.idCategoriaPadre;
        }
        if (
          this.getCategorias.imagen != "/storage/images/categorias/nada.png"
        ) {
          this.image = this.getCategorias.imagen;
        }
      });
    }
  },
